...
class RollbackportalWeb(Core):
    ...
    def run(self):
        ...
        rollback_backup_path = os.path.join(
            ...
            f'rollback_backup/{datetime.now().strftime("%Y%m%d")}',
        )
        time_str = datetime.now().strftime("%Y%m%d%H%M")
        name = f"{SERVICE_NAME}.back-{time_str}-{random.randint(1, 120)}"
        backup_file = os.path.join(rollback_backup_path, name)
        # 移动目录
        cmd_str = f"mkdir -p {rollback_backup_path} && mv {base_dir_path} {backup_file} && mv {self.para.backup_path} {base_dir_path} && echo {SERVICE_NAME}回滚成功, 备份路径:{backup_file}"
